...
- 未知路径返回 `404`。
- 剪贴板请求体超过 10MB 返回 `413`。
- 未捕获异常返回 `500`，且不会影响服务端处理后续请求。
- 发送端收到 `>= 300` 的 HTTP 状态码时抛出错误。
- 发送端连接远端超时为 10 秒。
- 剪贴板发送响应超时为 15 秒。
- 文件发送上传/响应超时为基础 60 秒，并按文件大小放宽：按至少 1 MB/s 的速率估算，每 MB 追加 1 秒，避免大文件被固定超时切断。
