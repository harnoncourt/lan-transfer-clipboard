...
- **本地网络权限**：首次启动的弹窗必须允许；误拒后在 设置 → 隐私与安全性 → 本地网络 中重新开启 LAN Transfer。权限被拒时发送心跳会静默失败，表现为一直 `0 在线`。
- **不受信任的开发者**：开发签名安装的应用首次打开如被拦截，在 设置 → 通用 → VPN 与设备管理 中信任证书。
- **证书过期**：个人开发证书签名的应用 7 天后无法打开，重新执行 `flutter install` 即可。
- **保持前台**：iOS 进入后台后系统会暂停网络收发，互传时保持应用在前台。

iOS 设备名显示为通用的 `iPad`/`iPhone` 是 iOS 16+ 的隐私限制，不是 bug。
...
- 移动端进入后台导致服务不可用。
- 网络发生切换。
- 远端连接建立后没有继续响应，发送端会超时报错：剪贴板 15 秒；文件为基础 60 秒并按大小放宽（每 MB 追加约 1 秒），大文件不会被固定超时切断。
...
- 如果传输中途失败（发送端退出、网络断开），接收端会删除写入了一半的文件，收件箱不会出现该记录；重新发送即可。
